import { Prisma, PrismaClient } from '@steadystart/prisma';
    ...
    $allModels: {
      paginate<T, A extends Prisma.Args<T, 'findMany'>, R = Prisma.Result<T, A, 'findMany'>>(
        ...
        return async ({ page, size }: PaginationArgs) => {
          ...
          const [rows, totalSize] = await prisma.$transaction([context.findMany({ ...args, take, skip }), context.count(args)]);
...
export type PaginatedPrisma = typeof prisma;
